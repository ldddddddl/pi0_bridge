...
# import openpi.bridgevla.RLBench.utils.peract_utils_rlbench as rlbench_utils
# import openpi.bridgevla.utils.rvt_utils as rvt_utils
...
class Real_Dataset(torch.utils.data.Dataset):
    def __init__(self, data_path, device, cameras, ep_per_task=10, output_arm_flag=False):
        self.device = device
    ...
    def convert_pcd_to_base(self, extrinsic_path, type="3rd", pcd=[]):
            ...
            if os.path.isdir(task_path):
                for episode_num in tqdm(os.listdir(task_path)):
                    ...
                    for step in range(num_steps - 1):
                        ...
                        sample["low_dim_state"] = np.concatenate([[current_gripper_state], [time]]).astype(np.float32)
                        ...
                        if "3rd" in self.cameras:
                            with open(os.path.join(rgb_3rd, f"{step}.pkl"), "rb") as f:
                                ...
                                sample["3rd"]["rgb"] = np.ascontiguousarray(
                                    sample["3rd"]["rgb"]
                                )  #   check it  the final image should be RGB
                            ...
                            with open(os.path.join(pcd_3rd, f"{step}.pkl"), "rb") as f:
                                ...
                                sample["3rd"]["pcd"] = self.convert_pcd_to_base(
                                    pcd=sample["3rd"]["pcd"],
                                    type="3rd",
                                ...
                                # # convert pcd to rgb image feature
                                # obs, pcd = rlbench_utils._preprocess_inputs(sample, ['3rd'])
                                # pc, img_feat = rvt_utils.get_pc_img_feat(obs=obs, pcd=pcd)
                                # img_aug = 0.0
                                # sample["3rd"]["pcd2rgb"] = render()

                        if "wrist" in self.cameras:
                            assert False
                            with open(os.path.join(rgb_wrist, f"{step}.pkl"), "rb") as f:
                                sample["wrist"]["rgb"] = pickle.load(f)
                                sample["wrist"]["rgb"] = np.ascontiguousarray(
                                    sample["wrist"]["rgb"][:, :, ::-1]
                                )  # BGR转RGB（H,W,C）
                            ...
                            with open(os.path.join(pcd_wrist, f"{step}.pkl"), "rb") as f:
                                ...
                                sample["wrist"]["pcd"] = self.convert_pcd_to_base(
                                    pcd=sample["wrist"]["pcd"], type="wrist"
                                )
...
if __name__ == "__main__":
    ...
    for data in dataset:
        ...
        save_pcd_with_gripper_ply(
            pcd, rgb, gripper_pose_xyz, save_path="/mnt/data1/3D_VLA/BridgeVLA/debug/pcd_with_gripper.ply"
        )
        a = 1
