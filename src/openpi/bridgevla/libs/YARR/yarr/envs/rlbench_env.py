...
def _get_cam_observation_elements(camera: CameraConfig, prefix: str, channels_last):
    ...
    if camera.point_cloud:
        elements.append(ObservationElement("%s_point_cloud" % prefix, shape, np.float32))
        elements.append(ObservationElement("%s_camera_extrinsics" % prefix, (4, 4), np.float32))
        elements.append(ObservationElement("%s_camera_intrinsics" % prefix, (3, 3), np.float32))
    if camera.depth:
...
def _observation_elements(observation_config, channels_last) -> list[ObservationElement]:
    ...
    elements.extend(
        _get_cam_observation_elements(observation_config.left_shoulder_camera, "left_shoulder", channels_last)
    )
    elements.extend(
        _get_cam_observation_elements(observation_config.right_shoulder_camera, "right_shoulder", channels_last)
    )
    elements.extend(_get_cam_observation_elements(observation_config.front_camera, "front", channels_last))
    elements.extend(_get_cam_observation_elements(observation_config.wrist_camera, "wrist", channels_last))
    return elements
...
class RLBenchEnv(Env):
    def __init__(
        ...
        self._rlbench_env = Environment(
            action_mode=action_mode, obs_config=observation_config, dataset_root=dataset_root, headless=headless
        )
        self._task = None
