...
class _IndependentEnvRunner(_EnvRunner):
    ...
    def _run_eval_independent(
        ...
        if self._save_metrics:
            ...
            writer = LogWriter(self._logdir, True, True, env_csv=csv_file)
        ...
        for n_eval in range(self._num_eval_runs):
            ...
            for ep in range(self._eval_episodes):
                ...
                if rec_cfg.enabled:
                    ...
                    record_file = os.path.join(
                        seed_path,
                        "videos",
                        "%s_w%s_s%s_%s.mp4" % (task_name, weight_name, eval_demo_seed, "succ" if success else "fail"),
                    )
            ...
            if len(summaries) > 0:
                if multi_task:
                    ...
                    task_score = [s.value for s in summaries if "eval_envs/return" in s.name][0]
            else:
                task_score = "unknown"
