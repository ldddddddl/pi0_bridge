...
class EnvRunner:
    def __init__(
        ...
        train_env: Env,
        agent: Agent,
        train_replay_buffer: ReplayBuffer | list[ReplayBuffer],
        num_train_envs: int,
        num_eval_envs: int,
        ...
        eval_replay_buffer: ReplayBuffer | list[ReplayBuffer] | None = None,
        ...
        self._train_replay_buffer = (
            train_replay_buffer if isinstance(train_replay_buffer, list) else [train_replay_buffer]
        )
    ...
    def _update(self):
        ...
        with self._internal_env_runner.write_lock:
            self._agent_summaries = list(self._internal_env_runner.agent_summaries)
            if (
                self._num_eval_episodes_signal.value % self._eval_episodes == 0
                and self._num_eval_episodes_signal.value > 0
            ):
            ...
            for name, transition, eval in self._internal_env_runner.stored_transitions:
                ...
                if add_to_buffer:
                    ...
                    if transition.terminal:
                        rb.add_final(**transition.final_observation)
                new_transitions[name] += 1
                self._new_transitions["eval_envs" if eval else "train_envs"] += 1
                self._total_transitions["eval_envs" if eval else "train_envs"] += 1
                if self._stat_accumulator is not None:
    ...
    def _run(self, save_load_lock):
        self._internal_env_runner = _EnvRunner(
            ...
            self._eval_from_eps_number,
            self._episode_length,
            self._kill_signal,
            self._step_signal,
            self._num_eval_episodes_signal,
            self._eval_epochs_signal,
            self._eval_report_signal,
            self.log_freq,
                ...
                if p.exitcode is not None:
                    ...
                    if p.exitcode != 0:
                        ...
                        if n_failures > self._max_fails:
                            logging.error(
                                "Env %s failed too many times (%d times > %d)" % (p.name, n_failures, self._max_fails)
                            )
                            raise RuntimeError("Too many process failures.")
                        logging.warning(
                            "Env %s failed (%d times <= %d). restarting" % (p.name, n_failures, self._max_fails)
                        )
