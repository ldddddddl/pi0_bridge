...
class OfflineTrainRunner:
    ...
    def _save_model(self, i):
        ...
        # remove oldest save
        prev_dir = os.path.join(self._weightsdir, str(i - self._save_freq * self._num_weights_to_keep))
        ...
        for i in range(start_iter, self._iterations):
            ...
            if self._rank == 0:
                if log_iteration and self._writer is not None:
                    ...
                    logging.info(
                        f"Train Step {i:06d} | Loss: {loss:0.5f} | Sample time: {sample_time:0.6f} | Step time: {step_time:0.4f}."
                    )
