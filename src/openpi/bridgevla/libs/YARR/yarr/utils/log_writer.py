...
from yarr.agents.agent import TextSummary
    ...
    def add_scalar(self, i, name, value):
        ...
        if self._csv_logging:
            if "env" in name or "eval" in name or "test" in name:
                ...
                self._env_row_data[name] = value.item() if isinstance(value, torch.Tensor) else value
            else:
                if len(self._train_row_data) == 0:
                    self._train_row_data["step"] = i
                self._train_row_data[name] = value.item() if isinstance(value, torch.Tensor) else value
            ...
            try:
                if isinstance(summary, ScalarSummary):
                    ...
                    if isinstance(summary, HistogramSummary):
                        self._tf_writer.add_histogram(summary.name, summary.value, i)
                        ...
                        # Only grab first item in batch
                        v = summary.value if summary.value.ndim == 5 else np.array([summary.value])
                        self._tf_writer.add_video(summary.name, v, i, fps=summary.fps)
            ...
            with open(self._train_csv_file, mode="a+") as csv_f:
                ...
                if should_write_train_header:
                    if self._train_field_names is None:
                        writer.writeheader()
                    elif not np.array_equal(self._train_field_names, self._train_row_data.keys()):
                        # Special case when we are logging faster than new
                        # summaries are coming in.
                        missing_keys = list(set(self._train_field_names) - set(self._train_row_data.keys()))
            ...
            with open(self._env_csv_file, mode="a+") as csv_f:
                ...
                if should_write_env_header:
                    if self._env_field_names is None:
                        writer.writeheader()
                    elif not np.array_equal(self._env_field_names, self._env_row_data.keys()):
                        # Special case when we are logging faster than new
                        # summaries are coming in.
                        missing_keys = list(set(self._env_field_names) - set(self._env_row_data.keys()))
                        for mk in missing_keys:
