...
class _SimpleAccumulator(StatAccumulator):
    def __init__(self, prefix, eval_video_fps: int = 30, mean_only: bool = True):
        self._prefix = prefix
    ...
    def _get(self) -> list[Summary]:
        ...
        sums.append(ScalarSummary("%s/total_transitions" % self._prefix, self._transitions))
...
class SimpleAccumulator(StatAccumulator):
    def __init__(self, eval_video_fps: int = 30, mean_only: bool = True):
        self._train_acc = _SimpleAccumulator("train_envs", eval_video_fps, mean_only=mean_only)
        self._eval_acc = _SimpleAccumulator("eval_envs", eval_video_fps, mean_only=mean_only)
...
class MultiTaskAccumulator(StatAccumulator):
    def __init__(
        ...
        self._train_accs = [
            _SimpleAccumulator("%s%d/envs" % (train_prefix, i), eval_video_fps, mean_only=mean_only)
            for i in range(num_tasks)
        ]
        self._eval_accs = [
            _SimpleAccumulator("%s%d/envs" % (eval_prefix, i), eval_video_fps, mean_only=mean_only)
            for i in range(num_tasks)
        ]
        self._train_accs_mean = _SimpleAccumulator(
            "%s_summary/envs" % train_prefix, eval_video_fps, mean_only=mean_only
        )
