...
def project_points_3d_to_pixels(
    pc: torch.Tensor, inv_cam_poses: torch.Tensor, intrinsics: torch.Tensor, orthographic: bool
):
    """
...
def get_batch_pixel_index(pc_px: torch.Tensor, img_height: int, img_width: int):
    ...
    """
    # batch_idx
    batch_idx = torch.arange(pc_px.shape[0], device=pc_px.device, dtype=torch.long)[
        :, None
    ]  # .repeat((1, pc_px.shape[1])).
    pix_off = 0.0
...
def lookat_to_cam_pose(eye, at, up=[0, 0, 1], device="cpu"):
    ...
    camera_right = F.normalize(
        torch.cross(camera_view, torch.tensor(up, dtype=torch.float32, device="cpu"), dim=0), dim=0
    )
...
def fov_and_size_to_intrinsics(fov, img_size, device="cpu"):
    ...
    intrinsics = torch.tensor([[fx, 0, img_h / 2], [0, fy, img_w / 2], [0, 0, 1]], dtype=torch.float, device=device)
    return intrinsics
