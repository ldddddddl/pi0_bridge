...
sys.path.append("/home/ldl/vla/BridgeVLA1/finetune/BridgeVLA1/finetune/libs")
...
class PreprocessAgent(Agent):
    ...
    def update_summaries(self) -> list[Summary]:
        ...
        sums = [
            ...
            HistogramSummary("%s/low_dim_state" % prefix, self._replay_sample["low_dim_state"]),
            HistogramSummary("%s/low_dim_state_tp1" % prefix, self._replay_sample["low_dim_state_tp1"]),
            ScalarSummary("%s/low_dim_state_mean" % prefix, self._replay_sample["low_dim_state"].mean()),
            ScalarSummary("%s/low_dim_state_min" % prefix, self._replay_sample["low_dim_state"].min()),
            ScalarSummary("%s/low_dim_state_max" % prefix, self._replay_sample["low_dim_state"].max()),
            ScalarSummary("%s/timeouts" % prefix, self._replay_sample["timeout"].float().mean()),
        ]
        ...
        if "sampling_probabilities" in self._replay_sample:
            sums.extend(
                [
                    HistogramSummary("replay/priority", self._replay_sample["sampling_probabilities"]),
                ]
            )
