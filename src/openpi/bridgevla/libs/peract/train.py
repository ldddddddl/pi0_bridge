...
def main(cfg: DictConfig) -> None:
    ...
    obs_config = create_obs_config(cfg.rlbench.cameras, cfg.rlbench.camera_resolution, cfg.method.name)
    ...
    elif cfg.framework.start_seed == -1 and len(list(filter(lambda x: "seed" in x, os.listdir(cwd)))) > 0:
        # unspecified seed; use largest existing seed plus one
        largest_seed = max([int(n.replace("seed", "")) for n in list(filter(lambda x: "seed" in x, os.listdir(cwd)))])
