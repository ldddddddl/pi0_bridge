...
from yarr.agents.agent import HistogramSummary
...
class Actor(nn.Module):
    ...
    def _rescale_actions(self, x):
        return 0.5 * (x + 1.0) * (self._action_min_max[1] - self._action_min_max[0]) + self._action_min_max[0]
...
class NextBestPoseAgent(Agent):
    ...
    def build(self, training: bool, device: torch.device = None):
        ...
        self._actor = Actor(self._actor_network, action_min_max).to(device).train(training)
        ...
        self._q = QFunction(self._critic_network, self._shared_network, self._q_conf).to(device).train(training)
        if training:
            ...
            self._crop_shape_t = torch.tensor([list(self._crop_shape)], dtype=torch.int32, device=device)
            ...
            if self._alpha_auto_tune:
                self._log_alpha = torch.tensor(
                    (np.log(self._alpha)), dtype=torch.float, requires_grad=True, device=device
                )
                if training:
                    self._alpha_optimizer = torch.optim.Adam([self._log_alpha], lr=self._alpha_lr)

            self._critic_optimizer = torch.optim.Adam(
                self._q.parameters(), lr=self._critic_lr, weight_decay=self._critic_weight_decay
            )
            self._actor_optimizer = torch.optim.Adam(
                self._actor.parameters(), lr=self._actor_lr, weight_decay=self._actor_weight_decay
            )

            logging.info("# NBP Critic Params: %d" % sum(p.numel() for p in self._q.parameters() if p.requires_grad))
            logging.info("# NBP Actor Params: %d" % sum(p.numel() for p in self._actor.parameters() if p.requires_grad))
        else:
    ...
    def _extract_crop(self, pixel_action, observation):
        ...
        top_left_corner = torch.clamp(pixel_action - self._crop_shape[0] // 2, 0, h - self._crop_shape[1])
    ...
    def _preprocess_inputs(self, replay_sample, pixel_action, pixel_action_tp1):
        observations = [
            self._extract_crop(pixel_action, replay_sample["%s_rgb" % self._camera_name]),
            self._extract_crop(pixel_action, replay_sample["%s_point_cloud" % self._camera_name]),
        ]
        tp1_observations = [
            self._extract_crop(pixel_action_tp1, replay_sample["%s_rgb_tp1" % self._camera_name]),
            self._extract_crop(pixel_action_tp1, replay_sample["%s_point_cloud_tp1" % self._camera_name]),
        ]
        return observations, tp1_observations

    def _clip_action(self, a):
        return torch.min(torch.max(a, self._action_min_max_t[0:1]), self._action_min_max_t[1:2])
    ...
    def _update_critic(self, replay_sample: dict) -> None:
        ...
        pixel_action_tp1 = replay_sample["%s_pixel_coord_tp1" % self._camera_name][:, -1]

        if self._crop_augmentation:
            shifted = (
                (torch.rand_like(pixel_action.float()) * self._crop_shape_t).int() - self._crop_shape_t // 2
        ...
        observations, tp1_observations = self._preprocess_inputs(replay_sample, pixel_action, pixel_action_tp1)
        ...
        with torch.no_grad():
            ...
            q1_pi_tp1_targ, q2_pi_tp1_targ, _, _ = self._q_target(tp1_observations, robot_state_tp1, pi_tp1)

            min_q_pi_targ = torch.min(q1_pi_tp1_targ[:, 0], q2_pi_tp1_targ[:, 0])
            next_value = min_q_pi_targ - self.alpha * logp_pi_tp1
            q_backup = reward.unsqueeze(-1) + (self._gamma**self._nstep) * (1.0 - terminal.unsqueeze(-1)) * next_value
        ...
        self._grad_step(critic_loss, self._critic_optimizer, self._q.parameters(), self._critic_grad_clip)
    ...
    def _update_actor(self, replay_sample: dict) -> None:
        ...
        self._grad_step(pi_loss, self._actor_optimizer, self._actor.parameters(), self._actor_grad_clip)

    def _update_alpha(self):
        alpha_loss = -(self.alpha * (self._logp_pi + self._target_entropy).detach()).mean()
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        return {"priority": ((self._new_priority + pixel_agent_priority) / 2.0) ** REPLAY_ALPHA}
    ...
    def act(self, step: int, observation: dict, deterministic=False) -> ActResult:
        with torch.no_grad():
            ...
            observations = [
                self._extract_crop(act_res.action.unsqueeze(0), observation["%s_rgb" % self._camera_name]),
                self._extract_crop(act_res.action.unsqueeze(0), observation["%s_point_cloud" % self._camera_name]),
            ]
    ...
    def update_summaries(self) -> list[Summary]:
        summaries = [
            ...
            ImageSummary("%s/crops_tp1/rgb" % NAME, (self._crop_summary_tp1[0] + 1.0) / 2.0),
            ImageSummary("%s/crops_tp1/point_cloud" % NAME, self._crop_summary_tp1[1]),
        ]

        for n, v in list(self._critic_summaries.items()) + list(self._actor_summaries.items()):
            summaries.append(ScalarSummary("%s/%s" % (NAME, n), v))

        for tag, param in list(self._q.named_parameters()) + list(self._actor.named_parameters()):
            summaries.append(HistogramSummary("%s/gradient/%s" % (NAME, tag), param.grad))
    ...
    def act_summaries(self) -> list[Summary]:
        summaries = [
            ImageSummary("%s/crops/act/rgb" % NAME, (self._act_crop_summaries[0] + 1.0) / 2.0),
            ImageSummary("%s/crops/act/point_cloud" % NAME, self._act_crop_summaries[1]),
        ]
    ...
    def load_weights(self, savedir: str):
        ...
        self._actor.load_state_dict(
            torch.load(os.path.join(savedir, "pose_actor.pt"), map_location=torch.device("cpu"))
        )
        self._q.load_state_dict(torch.load(os.path.join(savedir, "pose_q.pt"), map_location=torch.device("cpu")))
