...
class QAttentionAgent(Agent):
    ...
    def build(self, training: bool, device: torch.device = None):
        ...
        if training:
            ...
            self._optimizer = torch.optim.Adam(self._q.parameters(), lr=self._lr, weight_decay=self._weight_decay)
            logging.info("# Q-attention Params: %d" % sum(p.numel() for p in self._q.parameters() if p.requires_grad))
        else:
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        with torch.no_grad():
            ...
            target = reward.unsqueeze(1) + (self._gamma**self._nstep) * (1 - terminal.unsqueeze(1)) * q_tp1_targ
    ...
    def generate_heatmap(q_values, rgb_obs):
        ...
        heatmap = torch.cat([norm_q, torch.zeros_like(norm_q), torch.zeros_like(norm_q)])
    ...
    def update_summaries(self) -> list[Summary]:
        summaries = [
            ImageSummary(
                "%s/Q" % NAME,
                QAttentionAgent.generate_heatmap(self._qvalues.cpu(), ((self._rgb_observation + 1) / 2.0).cpu()),
            )
        ]
    ...
    def act_summaries(self) -> list[Summary]:
        return [
            ImageSummary(
                "%s/Q_act" % NAME,
                QAttentionAgent.generate_heatmap(self._act_qvalues.cpu(), ((self._rgb_observation + 1) / 2.0).cpu()),
            )
        ]

    def load_weights(self, savedir: str):
        self._q.load_state_dict(torch.load(os.path.join(savedir, "pixel_agent_q.pt"), map_location=torch.device("cpu")))
