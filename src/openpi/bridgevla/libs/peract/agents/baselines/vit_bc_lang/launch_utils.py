...
from helpers.network_utils import ViT
from helpers.network_utils import ViTLangAndFcsNet
...
def create_agent(
    camera_name: str, activation: str, lr: float, weight_decay: float, image_resolution: list, grad_clip: float
):
    vit = ViT(
        image_size=128,
        patch_size=8,
    ...
    bc_agent = ViTBCLangAgent(
        actor_network=actor_net, camera_name=camera_name, lr=lr, weight_decay=weight_decay, grad_clip=grad_clip
    )
