...
class BCLangAgent(Agent):
    ...
    def build(self, training: bool, device: torch.device = None):
        ...
        if training:
            ...
            logging.info("# Actor Params: %d" % sum(p.numel() for p in self._actor.parameters() if p.requires_grad))
        else:
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        self._grad_step(loss, self._actor_optimizer, self._actor.parameters(), self._grad_clip)
        self._summaries = {
            "pi/loss": loss,
            "pi/mu": mu.mean(),
        }
    ...
    def load_weights(self, savedir: str):
        self._actor.load_state_dict(torch.load(os.path.join(savedir, "bc_actor.pt"), map_location=torch.device("cpu")))
