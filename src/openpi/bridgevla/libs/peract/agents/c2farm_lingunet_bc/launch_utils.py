...
def create_replay(
    ...
    for cname in cameras:
        ...
        observation_elements.append(ObservationElement("%s_pixel_coord" % cname, (2,), np.int32))
    ...
    for depth in range(len(voxel_sizes)):
        observation_elements.append(ReplayElement("attention_coordinate_layer_%d" % depth, (3,), np.float32))
...
def _add_keypoints_to_replay(
    ...
    for k, keypoint in enumerate(episode_keypoints):
        ...
        for depth in range(len(voxel_sizes)):
            final_obs["attention_coordinate_layer_%d" % depth] = attention_coordinates[depth]
        for name in cameras:
