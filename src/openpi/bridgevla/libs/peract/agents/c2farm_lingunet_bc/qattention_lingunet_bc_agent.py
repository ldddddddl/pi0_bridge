...
class QAttentionLingUNetBCAgent(Agent):
    ...
    def build(self, training: bool, device: torch.device = None):
        ...
        self._q = (
            QFunction(self._unet3d, self._voxelizer, self._bounds_offset, self._rotation_resolution, device, training)
            .to(device)
            .train(training)
        )
    ...
    def act(self, step: int, observation: dict, deterministic=False) -> ActResult:
        ...
        q_ignore_collisions = (
            self._softmax_ignore_collision(q_ignore_collisions) if q_ignore_collisions is not None else None
        )
    ...
    def load_weights(self, savedir: str):
        ...
        state_dict = torch.load(os.path.join(savedir, "%s.pt" % self._name), map_location=device)
