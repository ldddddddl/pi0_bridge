...
from ...helpers.network_utils import Conv3DUpsampleBlock
...
class PerceiverVoxelLangEncoder(nn.Module):
    def __init__(
        ...
        get_latent_attn = lambda: PreNorm(
            latent_dim, Attention(latent_dim, heads=latent_heads, dim_head=latent_dim_head, dropout=attn_dropout)
        )
        ...
        if self.num_rotation_classes > 0:
            ...
            self.rot_grip_collision_ff = DenseBlock(
                self.final_dim,
                self.num_rotation_classes * 3 + self.num_grip_classes + self.num_collision_classes,
                None,
                None,
            )
