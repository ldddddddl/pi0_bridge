...
from .cameras import OrthographicCameras
from . import ops
from .profiler import PerfTimer
import torch
...
def _prep_render_batch_inputs(points, features, inv_poses, intrinsics, img_h: int, img_w: int, orthographic: bool):
    ...
    return (
        point_pixel_index.reshape([batch_size * num_points]).contiguous(),
        point_depths.reshape([batch_size * num_points]).contiguous(),
        features,
    )
...
class PointRenderer:
    ...
    @torch.no_grad()
    def splat_filter(
        self, batch_size, splat_radius, cameras, point_depths, depth_buf, image_buf, default_depth, splat_max_k
    ):
    ...
    def render_batch(
        ...
        r.render_feature_pointcloud_to_image(
            point_pixel_index, point_depths, features, image_buf, depth_buf, default_depth, default_color
        )
        ...
        if splat_radius is not None:
            self.splat_filter(
                batch_size, splat_radius, cameras, point_depths, depth_buf, image_buf, default_depth, splat_max_k
            )
        ...
        if aa_factor != 1:
            ...
            depth_out = torch.zeros((batch_size, img_h_out, img_w_out), device=self.device, dtype=torch.float32).fill_(
                default_depth
            )
            image_out = torch.zeros(
                (batch_size, img_h_out, img_w_out, num_channels), device=self.device, dtype=torch.float32
            )
            print(depth_buf.min(), depth_buf.max())

            r.aa_subsample(depth_buf, image_buf, depth_out, image_out, aa_factor, default_depth)
            self.timer.check("antialiasing")
            return image_out.reshape((batch_size, img_h_out, img_w_out, num_channels)), depth_out.reshape(
                (batch_size, img_h_out, img_w_out)
            )
