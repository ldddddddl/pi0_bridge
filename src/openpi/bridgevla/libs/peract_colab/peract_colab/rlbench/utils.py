...
def get_stored_demo(data_path, index):
    ...
    try:
        ...
        print("[Warning] You only have one variation. Make sure you are training on colossum")
    # with open(os.path.join(episode_path, VARIATION_NUMBER_PICKLE), 'rb') as f:
    #   obs.variation_number = pickle.load(f)
    ...
    for i in range(num_steps):
        ...
        obs[i].front_depth = image_to_float_array(
            Image.open(os.path.join(episode_path, "%s_%s" % (CAMERA_FRONT, IMAGE_DEPTH), IMAGE_FORMAT % i)), DEPTH_SCALE
        )
        ...
        obs[i].wrist_depth = image_to_float_array(
            Image.open(os.path.join(episode_path, "%s_%s" % (CAMERA_WRIST, IMAGE_DEPTH), IMAGE_FORMAT % i)), DEPTH_SCALE
        )
